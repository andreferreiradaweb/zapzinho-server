...
const bodySchema = z.object({
  ...
  warmupVariations: z.array(z.string()).optional(),
...
export async function createProspectingBroadcastController(
  ...
  try {
    const result = await makeCreateProspectingBroadcast().execute({
      userId,
      ...data,
    });
    ...
    const message =
      err instanceof Error ? err.message : "Erro ao criar disparo";
